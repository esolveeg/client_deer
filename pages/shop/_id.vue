            ...
            <v-row>
              ...
              <v-col cols="12" lg="6" >
                <div class="">
                    ...
                    <v-img v-else min-height="200" :src="product.productImage"></v-img>

                </div>
              </v-col>
              <!-- <v-col cols="2"></v-col> -->

              <v-col cols="12" class="product-description flex flex-column justify-center" lg="6" >   
                <h2 class="product-title larger mb-1">{{product.productName}}</h2>
                <h5 class="my-4">{{product.isbn}}</h5>
                <div class="flex flex-0 h-4">
                  <nuxt-link  v-for="group in product.groups" :key="group.id" :to="{name : `shop___${$i18n.locale}` , query: {group : group.id}}" class="category">{{group.groupName}}</nuxt-link>
                </div>
               <div class="mb-4">
                <nuxt-link :to="{name : `shop___${$i18n.locale}` , query: {author : product.author.id}}" class="author active">{{$t('by')}} : {{product.author.authorName}}</nuxt-link>
              </div>
                <p class="desc" v-if="$i18n.locale == 'ar' && product.ItemDesc">{{product.productDesc}}</p>
                <p class="p">
                  {{product.productDesc}}
                </p>
